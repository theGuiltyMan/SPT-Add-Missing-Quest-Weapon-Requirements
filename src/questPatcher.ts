                                ...
                                try 
                                {
                                    ...
                                    else 
                                    {
                                        if (!(questOverrides[questId] && questOverrides[questId].onlyUseWhiteListedWeapons) && newWeaponCondition.length > 1) 
                                        {
                                            ...
                                            if (notFoundWeapons.length > 0) 
                                            {
                                                const origConditionCount = newWeaponCondition.length;
                                                let logStr = `The following weapons were not found in database by the mod while processing quest ${questId}: ${notFoundWeapons.join(", ")}`;
                                                logStr += "\n The weapon might be a modded weapon that have added to the quest by another mod, but could not be found in the database.";
                                                logStr += "\n The mod will continue with available weapons."

                                                // remove not found weapons from the conditions to keep the calculations 
                                                notFoundWeapons.forEach(w => 
                                                {
                                                ...
                                                this.logger.log(`Removed not found weapons from conditions. Original count: ${origConditionCount}, New count: ${newWeaponCondition.length}`);
